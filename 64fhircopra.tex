...
\begin{longtable}{|l|p{5cm}|}
	\caption[Erkannte Konfigurationsvariablen durch durch die Anwendung einer Kombination von allgemeinen Teilwörtern]{Erkannte Konfigurationsvariablen bei der Anwendung der Kombination von allgemeinen Teilwörtern. Das \ac{fhir}-Profil \glqq Mittlerer Beatmungsdruck\grqq{} wurde drei Konfigurationsvariablen zugeordnet.}
...
Das Ergebnis des Pattern Matchings is in der Tabelle \texttt{mapping\_mii\_co6} (\ref{tab:mapping}) registriert. Dieser Datensatz wurde danach (\ref{sec:validmethode}) von den Fachleute der \ac{pdms}-Abteilung validiert.