...
\end{longtable}
 ...
 %der Konfigurationsvariablen mit den \ac{fhir}-Profilen in der \ac{db} in der Tabelle \texttt{mapping\_mii \_co6} (\ref{tab:mapping}) gespeichert. In dieser Tabelle befinden sich nur die Profile des Typs \glqq Observation\grqq{}, weil alle \ac{fhir}-Profile, die zugeordnet wurden, zu diesem Typ gehören. Andere Anmerkung ist, dass an dieser Stelle nicht berücksichtigt wurde, ob eine \glqq Observation\grqq{} zu der Gruppe der \glqq Blutdruck Generisch\grqq{} gehören oder nicht, sodass bei den gespeicherten Zuordnungen die zu dieser Gruppe nicht gehören, werden die charakteristischen Spalten für die Attributen der Blutdruckmessung immer leer sein.