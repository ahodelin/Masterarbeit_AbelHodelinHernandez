...
select 
  initcap(description) profile_name,
  name conf_var_name ,
...
where lower(name) in (select lower(profile_name) from mii_copra.mii_icu mi) 
or lower(description) in (select lower(profile_name) from mii_copra.mii_icu mi)
and deleted = 'NULL'
...
/*
x Sauerstoffsaettigung im Blut postduktal durch Pulsoxymetrie
x Sauerstoffsaettigung im Blut preductal durch Pulsoxymetrie
- Sauerstoffsaettigung im arteriellen Blut Per Pulsoxymetrie


update mii_copra.mii_icu 
  set mapped = false where profile_name in ('Sauerstoffsaettigung im Blut postduktal durch Pulsoxymetrie', 'Sauerstoffsaettigung im Blut preductal durch Pulsoxymetrie');
 */  
...
select 
  ...
  0 matching
from mii_copra.co6_config_variables
where (name ~* 'PV' or description ~* 'PV')
and deleted isnull
and id not in (select conf_var_id from mii_copra.mapping_mii_co6 mmc)
order by name  
;
...
update mii_copra.mii_icu 
  set mapped = true where profile_name in (select profile_name from mii_copra.mapping_mii_co6);
 
 
-- falta
select 
  distinct icu.profile_name, icu.loinc, l.mapped  
from mii_copra.loinc l
join mii_copra.mii_icu icu
  on icu.loinc ~ l.loinc 
where l.mapped isnull
--and profile_unit notnull 
order by icu.loinc;
